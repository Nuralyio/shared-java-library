...
public class ExampleACLResource {
    ...
    /**
     * Example 2: Assign a user as "Editor" to a document at the org level
     * POST /api/v1/examples/assign-editor-role
     */
    @POST
    @Path("/assign-editor-role")
    @RequiresPermission(permissionType = "admin", resourceType = "organization", resourceId = "#{organizationId}")
    public Response assignEditorRole(AssignEditorRequest request) {
        try {
            ...
            // Grant all editor permissions to the user for all organization resources
            List<Resource> orgResources = Resource.findByOrganization(request.organizationId);
            int grantsCreated = 0;
            ...
            return Response.ok(new AssignRoleResponse(
                "User assigned as Editor for organization",
                grantsCreated,
                editorRole.name
            )).build();
    ...
    public Response createCollaborativeWorkspace(CreateWorkspaceRequest request) {
        try {
            ...
            teamPolicy.policyType = SharePolicyType.ORGANIZATION;
